		...
		switch(step) {
			...
			case 1:
				...
				if(qmsOn > 22 && qmsOn < 26) {
					//6ms on, beacon SYNC.
					workingBuffer.headerType = eLTTO_IR_HEADERTYPE_BEACON;
				} else if(qmsOn > 10 && qmsOn < 14) {
					//3ms on, data SYNC.
					workingBuffer.headerType = eLTTO_IR_HEADERTYPE_NORMAL;
				} else {
			...
			case 10: //Ninth data bit
				...
				if(qmsOff > 6 && qmsOff < 10) {
					...
					switch(step) {
						case 6:
							workingBuffer.bitCount = eLTTO_IR_BITCOUNT_5;
							break;
						case 8:
							workingBuffer.bitCount = eLTTO_IR_BITCOUNT_7;
							break;
						case 9:
							workingBuffer.bitCount = eLTTO_IR_BITCOUNT_8;
							break;
						case 10:
							workingBuffer.bitCount = eLTTO_IR_BITCOUNT_9;
							break;
					}
					step = 0;
